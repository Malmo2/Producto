...
const TIMER_SNAPSHOT_KEY = "timeSnapshotV1";
...
function buildInitialState() {
  ...
  if (!Number.isFininte(timeLeft) || timeLeft < 0) timeLeft = 0;
...
function shouldOpenPopupOnLoad() {
  ...
  const isFinished =
    ...
    (Boolean(snap.isRunning) === false) & hasStart;
  return isFinished;
}
...
export function TimerProvider({ children }) {
  ...
  }, [state.mode, state.customMinutes, state.timeLeft, state.isRunning, state.startTime, state.endTime]);
  ...
  useEffect(() => {
    const raw = localStorage.getItem(TIMER_SNAPSHOT_KEY);
    if(!raw) return;
  ...
  const api = useMemo(() => ({
    ...
    setModeWithDefaults,
    setModeAndMinutes,
    endSession: () => { dispatch({ type: "PAUSE_TIMER" }); setIsSessionPopupOpen(true); },
